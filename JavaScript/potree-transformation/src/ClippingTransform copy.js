...
import png from '../../../assets/eye_2.png'

export class ClippingTransform {
    constructor(camera, renderer, handler, controls = null) {
        this.camera = camera
        ...
        this.selection_changed = (e) => {
            for (let selected of this.selection) {
                this.inputHandler.blacklist.delete(selected)
            }

            this.selection = e.selection

            for (let selected of this.selection) {
                this.inputHandler.blacklist.add(selected)
            }
        }
    ...
    initializeTranslationHandles() {
        ...
        for (let handleName of Object.keys(this.translationHandles)) {
            ...
            pickMaterial.visible = this.showPickVolumes
            ...
            node.setOpacity = (target) => {
                ...
                let t = new Tween(opacity).to({ x: target }, 100)
                t.onUpdate(() => {
    ...
    dragTranslationHandle(e) {
...

        {
            ...
            if (I) {
                let iOnLine = drag.line.closestPointToPoint(
                    I,
                    false,
                    new Vector3()
                )
    ...
    dragScaleHandle(e) {
...

        {
            ...
            if (I) {
                ...
                let toObjectSpace = new Matrix4().getInverse(
    ...
    initializeFocusHandles() {
        ...
        let texture = new TextureLoader().load(png)
    ...
    update() {
        if (this.selection.length === 1) {
            ...
            let camera = this.camera
            let domElement = this.domElement
            let mouse = this.inputHandler.mouse
            ...
            for (let handleName of Object.keys(this.handles)) {
                ...
                let rotInv = new Matrix4().getInverse(rot)
            ...
            if (!this.dragging) {
                ...
                let tObject = new Matrix4().getInverse(tWorld)
...

            {
                let axisScale = (alignment) => {
                    let transformed = new Vector3(...alignment).applyMatrix4(
                        selected.matrixWorld
                    )
                    let distance = transformed.distanceTo(
                        selected.getWorldPosition(new Vector3())
                    )
    ...
    dispose() {
        this.inputHandler.unregisterInteractiveScene(this.scene)
        this.inputHandler.removeEventListener(
            'selection_changed',
            this.selection_changed
        )
        Utils.disposeAll(this.scene)
        this.setActiveHandle(null)
    }
}
